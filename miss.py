...
def z_list(proxy, page):
    ...
    url = (
        proxy
        + "https://18av.mm-cg.com/zh/uncensored_makersr/29/Tokyo%20Hot%20(%E6%9D%B1%E4%BA%AC%E7%86%B1)/{0}.html".format(
            page_num
        )
    )
...
def jrate():
    ...
    for _ in range(3):
        ...
        page_url_full = (
            page_url[vadio_num].ele("t:div@@class:text-truncate")("t:a").link
        )
...
def addr_miss(data_list: list, proxy_url) -> None:
    for _ in range(4):
        try:
            ...
            base_url = proxy_url
            tokey_url = base_url + "https://missav.com/dm16/tokyohot?page={0}".format(
                tokey_index
            )
            high_url = (
                base_url
                + "https://missav.com/search/%E9%AB%98%E5%A6%B9?sort=released_at&page={0}".format(
                    high_index
                )
            )
            ...
            for url in url_list:
                ...
                for _ in range(3):
                    ...
                    url_list = page.eles(
                        'xpath://div[contains(@class,"my-2")]/a/@href'
                    )[num_vid].strip()
...
def addr_pig(data_list: list, pl_url) -> None:
...
def addr_18tv(data_list: list, de_url) -> None:
    for _ in range(4):
        try:
...
def addr_grate(data_list: list) -> None:
    for _ in range(4):
        try:
            j_data = jrate()
...
def miss_main(proxy_url, wh_url, chat_id, pl_url, de_url):
    ...
    # 主线程等待所有子线程完成
    wait(tasks, timeout=140, return_when=ALL_COMPLETED)

    # 格式转换
    box = convert_to_m3u(data_list)
    # 保存
